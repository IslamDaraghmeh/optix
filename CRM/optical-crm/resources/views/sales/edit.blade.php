    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('app.edit_sale') }} #{{ str_pad($sale->id, 6, '0', STR_PAD_LEFT) }}
            </h2>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('sales.update', $sale) }}" id="saleForm" class="space-y-6">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select name="patient_id" id="patient_id"
                                    ...
                                    @foreach($patients as $patient)
                                        <option value="{{ $patient->id }}" {{ old('patient_id', $sale->patient_id) == $patient->id ? 'selected' : '' }}>
                                    ...
                                    <div class="item-row border rounded-lg p-4 bg-gray-50">
                                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                            ...
                                            <div class="flex items-end">
                                                <button type="button"
                                                    class="remove-item bg-red-500 hover:bg-red-600 text-white px-3 py-2 rounded-lg transition-colors {{ count($sale->items) > 1 ? '' : 'hidden' }}">
                                                    {{ __('app.remove') }}
                                                </button>
                                            </div>
                                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.01" name="paid_amount" id="paid_amount"
                                        value="{{ old('paid_amount', $sale->paid_amount) }}" min="0" required
