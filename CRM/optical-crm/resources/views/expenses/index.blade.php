<x-app-layout>
        ...
        <div class="flex justify-between items-center animate-fadeInUp">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p class="text-white/90 text-sm mt-1 font-medium">{{ __('Track business expenses and costs') }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-2xl p-6 text-white shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-white/95 text-sm font-semibold uppercase tracking-wide">{{ __('Total Expenses') }}</p>
                            <p class="text-4xl font-bold mt-2 text-white">{{ \App\Models\Expense::count() }}</p>
                        </div>
                ...
                <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-6 text-white shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-white/95 text-sm font-semibold uppercase tracking-wide">{{ __('This Month') }}</p>
                            <p class="text-4xl font-bold mt-2 text-white">{{ number_format(\App\Models\Expense::whereMonth('created_at', now()->month)->sum('amount'), 0) }}</p>
                        </div>
                        <div class="bg-white/25 p-3 rounded-xl backdrop-blur-sm">
                            <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-6 text-white shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-white/95 text-sm font-semibold uppercase tracking-wide">{{ __('Largest Category') }}</p>
                            <p class="text-4xl font-bold mt-2 text-white">{{ \App\Models\Expense::select('category')->groupBy('category')->orderByRaw('COUNT(*) DESC')->value('category') ?? 'N/A' }}</p>
                        </div>
                        <div class="bg-white/25 p-3 rounded-xl backdrop-blur-sm">
                            <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-6 text-white shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-white/95 text-sm font-semibold uppercase tracking-wide">{{ __('Average Expense') }}</p>
                            <p class="text-4xl font-bold mt-2 text-white">{{ \App\Models\Expense::count() > 0 ? number_format(\App\Models\Expense::avg('amount'), 0) : 0 }}</p>
                        </div>
                        <div class="bg-white/25 p-3 rounded-xl backdrop-blur-sm">
                            <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="card-primary overflow-hidden rounded-2xl shadow-2xl border-2 border-primary-100">
                ...
                <div class="p-6 bg-white">
                    <div class="overflow-x-auto">
                        <table id="expensesTable" class="min-w-full divide-y divide-gray-200 display nowrap" style="width:100%">
                            <thead class="bg-gradient-to-r from-primary-600 to-primary-700">
                                <tr>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">{{ __('app.title') }}</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">{{ __('app.category') }}</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">{{ __('app.amount') }}</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">{{ __('app.payment_method') }}</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">{{ __('app.vendor_info') }}</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">{{ __('app.expense_date') }}</th>
    ...
    <style>
        #expensesTable_wrapper .dt-buttons {
            margin-bottom: 1rem;
            float: left;
        }
        #expensesTable_wrapper .dt-button {
        ...
        #expensesTable_wrapper .dt-button:hover {
            transform: translateY(-2px);
            box-shadow: 0 4px 8px rgba(0, 0, 0, 0.15);
        }
        #expensesTable_wrapper .dataTables_filter {
            float: right;
        }
        ...
        #expensesTable_wrapper .dataTables_length select {
            border: 1px solid #d1d5db;
            border-radius: 0.5rem;
