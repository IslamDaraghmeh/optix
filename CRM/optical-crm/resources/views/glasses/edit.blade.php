    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('app.edit_glass') }} - {{ $glass->patient->name }}
            </h2>
            <div class="flex space-x-2">
                <a href="{{ route('glasses.show', $glass) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition-colors">
                    {{ __('app.view') }}
                </a>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('glasses.update', $glass) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select name="patient_id" id="patient_id" required
                                    ...
                                    @foreach($patients as $patient)
                                        <option value="{{ $patient->id }}" {{ old('patient_id', $glass->patient_id) == $patient->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="text" name="lens_type" id="lens_type" value="{{ old('lens_type', $glass->lens_type) }}" required
                            ...
                            <div>
                                ...
                                <input type="text" name="frame_type" id="frame_type" value="{{ old('frame_type', $glass->frame_type) }}" required
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" name="price" id="price" value="{{ old('price', $glass->price) }}" required
                                       placeholder="0.00"
                            ...
                            <div>
                                ...
                                <select name="status" id="status" required
                                    ...
                                    <option value="pending" {{ old('status', $glass->status) == 'pending' ? 'selected' : '' }}>{{ __('app.pending') }}</option>
                                    <option value="ready" {{ old('status', $glass->status) == 'ready' ? 'selected' : '' }}>{{ __('app.ready') }}</option>
                                    <option value="delivered" {{ old('status', $glass->status) == 'delivered' ? 'selected' : '' }}>{{ __('app.delivered') }}</option>
                                </select>
                                @error('status')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="flex justify-end space-x-4">
                            <a href="{{ route('glasses.show', $glass) }}"
