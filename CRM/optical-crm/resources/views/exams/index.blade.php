<x-app-layout>
        ...
        <div class="flex justify-between items-center animate-fadeInUp">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p class="text-white/90 text-sm mt-1 font-medium">{{ __('Manage patient eye examination records') }}</p>
                </div>
            </div>
        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                ...
                <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl p-6 text-white shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-4xl font-bold mt-2 text-white">{{ \App\Models\Exam::whereMonth('created_at', now()->month)->count() }}</p>
                        </div>
                        <div class="bg-white/25 p-3 rounded-xl backdrop-blur-sm">
                            <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-6 text-white shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-white/95 text-sm font-semibold uppercase tracking-wide">{{ __('Patients with Exams') }}</p>
                            <p class="text-4xl font-bold mt-2 text-white">{{ \App\Models\Exam::distinct('patient_id')->count('patient_id') }}</p>
                        </div>
                        <div class="bg-white/25 p-3 rounded-xl backdrop-blur-sm">
                            <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-6 text-white shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-white/95 text-sm font-semibold uppercase tracking-wide">{{ __('Average per Patient') }}</p>
                            <p class="text-4xl font-bold mt-2 text-white">{{ \App\Models\Patient::count() > 0 ? number_format(\App\Models\Exam::count() / \App\Models\Patient::count(), 1) : 0 }}</p>
                        </div>
                        <div class="bg-white/25 p-3 rounded-xl backdrop-blur-sm">
                            <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="card-primary overflow-hidden rounded-2xl shadow-2xl border-2 border-primary-100">
                ...
                <div class="p-6 bg-white">
                    <div class="overflow-x-auto">
                        <table id="examsTable" class="min-w-full divide-y divide-gray-200 display nowrap" style="width:100%">
                            <thead class="bg-gradient-to-r from-primary-600 to-primary-700">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">{{ __('app.right_eye') }}</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">{{ __('app.left_eye') }}</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">{{ __('app.exam_date') }}</th>
    ...
    <style>
        #examsTable_wrapper .dt-buttons {
            margin-bottom: 1rem;
            float: left;
        }
        #examsTable_wrapper .dt-button {
        ...
        #examsTable_wrapper .dt-button:hover {
            transform: translateY(-2px);
            box-shadow: 0 4px 8px rgba(0, 0, 0, 0.15);
        }
        #examsTable_wrapper .dataTables_filter {
            float: right;
        }
        #examsTable_wrapper .dataTables_filter input {
            border: 1px solid #d1d5db;
            border-radius: 0.5rem;
            padding: 0.5rem 1rem;
            margin-left: 0.5rem;
        }
        #examsTable_wrapper .dataTables_length select {
            border: 1px solid #d1d5db;
            border-radius: 0.5rem;
    ...
    $(document).ready(function() {
        ...
        $('#examsTable').DataTable({
            ...
            columns: [
                ...
                { data: 'right_eye', name: 'right_eye' },
                { data: 'left_eye', name: 'left_eye' },
